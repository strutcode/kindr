      ...
      <div class="lg:w-80 flex-shrink-0">
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 sticky top-24">
          ...
          <div class="mb-6">
            ...
            <select v-model="filters.category" class="input" @change="handleFiltersChange">
          ...
          <div v-if="filters.category" class="mb-6">
            ...
            <select v-model="filters.subcategory" class="input" @change="handleFiltersChange">
...
<script setup lang="ts">
  ...
  const { debug, warn } = createLogger('RequestsView')
  ...
  const displayedRequests = computed(() => {
    if (
      ...
      (userLocation.value && locationStatus.value !== 'loading')
    ) {
  ...
  const currentLocationDisplay = computed(() => {
    ...
    if (locationStatus.value === 'custom') {
      ...
      return (
        ...
        `${userLocation.value.latitude.toFixed(4)}, ${userLocation.value.longitude.toFixed(4)}`
      )
    }
  ...
  const requestLocation = async () => {
    try {
      ...
      warn('Location access failed, using default location:', error)
  ...
  const handleLocationSelected = async (location: LocationSelection) => {
    debug('Location selected:', location)
  ...
  const fetchRequestsForLocation = async () => {
    ...
    try {
      localLoading.value = true
      debug('Fetching requests for location:', userLocation.value, 'radius:', filters.radius)
      ...
      debug(`Found ${requests.length} requests within ${filters.radius} miles`)
