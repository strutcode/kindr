...
CREATE TABLE IF NOT EXISTS listings (
  ...
  DELETE
    ...
    category text NOT NULL CHECK (
      category IN ('free-stuff', 'help-needed', 'skills-offered')
    ),
    ...
    expiry_seconds integer DEFAULT 604800 CHECK (
      -- 1 hour, 12 hours, 1 day, 3 days, 5 days, 7 days
      expiry_seconds IN (3600, 43200, 86400, 259200, 432000, 604800)
    ),
    created_at timestamptz DEFAULT now(),
    updated_at timestamptz DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS reviews (
  id uuid DEFAULT uuid_generate_v4() PRIMARY KEY,
  listing_id uuid REFERENCES listings(id) ON
  DELETE
    CASCADE NOT NULL,
    reviewer_id uuid REFERENCES users(id) ON
  DELETE
    CASCADE NOT NULL,
    reviewee_id uuid REFERENCES users(id) ON
  DELETE
    CASCADE NOT NULL,
...
ALTER TABLE
  listings ENABLE ROW LEVEL SECURITY;

ALTER TABLE
  reputation ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Anyone can read active listings" ON listings FOR
SELECT
  USING (active = true);

CREATE POLICY "Users can read own listings" ON listings FOR
SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can create listings" ON listings FOR
INSERT
...
CREATE POLICY "Users can delete own listings" ON listings FOR
DELETE
  USING (auth.uid() = user_id);

-- RLS Policies for reputation
CREATE POLICY "Anyone can read reputation" ON reputation FOR
SELECT
  USING (true);

CREATE POLICY "System can insert reputation" ON reputation FOR
INSERT
  WITH CHECK (true);

CREATE POLICY "System can update reputation" ON reputation FOR
UPDATE
  USING (true);

-- RLS Policies for reviews
CREATE POLICY "Users can read reviews about them" ON reviews FOR
SELECT
  USING (
...
CREATE
OR REPLACE FUNCTION update_reputation(
  user_id uuid,
  ...
  VALUES
    (user_id, positive_delta, negative_delta, 1) ON CONFLICT (user_id) DO
  UPDATE
  SET
...
CREATE POLICY "Users can delete own images" ON storage.objects FOR
DELETE
  TO authenticated USING (
    bucket_id = 'listing-images'
    AND auth.uid() :: text = (storage.foldername(name)) [ 1 ]
...
CREATE
OR REPLACE FUNCTION cleanup_orphaned_images() RETURNS void AS $$ BEGIN
  DELETE FROM
    storage.objects
  WHERE
    bucket_id = 'listing-images'
    AND NOT EXISTS (
      ...
      FROM
        listings
      WHERE
        listings.images @ > ARRAY [ storage.objects.name ]
    );
...
CREATE
OR REPLACE FUNCTION mark_expired_listings_inactive() RETURNS void AS $$ BEGIN
  UPDATE
    listings
  SET
    is_active = false
...
CREATE INDEX IF NOT EXISTS idx_listings_location_active_gist ON listings USING GIST (location)
WHERE
  active = true;

-- Function to get listings within map bounds using PostGIS
CREATE
OR REPLACE FUNCTION get_listings_in_bounds(
...
BEGIN
  ...
  IF north <= south
  OR east <= west THEN RAISE
  EXCEPTION
    'Invalid bounds: north=%, south=%, east=%, west=%',
    north,
    south,
    east,
    west;
...
OR north > 90
OR south < -90
OR south > 90 THEN RAISE
EXCEPTION
  'Invalid latitude bounds: north=%, south=%',
  north,
  south;
...
OR east > 180
OR west < -180
OR west > 180 THEN RAISE
EXCEPTION
  'Invalid longitude bounds: east=%, west=%',
  east,
  west;
...
IF west > east THEN -- Bounds cross the international date line
...
WHERE
  ...
  AND (
    ...
    OR r.subcategory = p_subcategory
  )
ORDER BY
  distance_meters ASC
LIMIT
